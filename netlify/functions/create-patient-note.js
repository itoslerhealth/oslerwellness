...
export async function handler(event) {
  ...
  try {
    ...
    const payload = { app_id: process.env.PLATO_APP_ID,"app-id": process.env.PLATO_APP_ID, app_secret: process.env.PLATO_APP_SECRET,"app-secret": process.env.PLATO_APP_SECRET, token };
    const ctxRes = await fetch("https://clinic.platomedical.com/apps/context", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
    const ctxText = await ctxRes.text();
    if (!ctxRes.ok) return { statusCode: ctxRes.status, headers, body: `Context failed: ${ctxText}` };
    let ctxObj = {}; try { ctxObj = JSON.parse(ctxText); } catch {}
    ...
    if (!patientId || !database) return { statusCode: 400, headers, body: "No patient/database in context" };
    ...
    const notePayload = { patient_id: patientId, title, note: html, status: "draft", folder: "Osler Wellness" };
    const r = await fetch(apiUrl, { method: "POST", headers: { "Authorization": `Basic ${auth}`, "Content-Type": "application/json", "Accept": "application/json" }, body: JSON.stringify(notePayload) });
  ...
  } catch (e) { return { statusCode: 500, headers, body: e?.message || "Create note failed" }; }
}
