...
export async function handler(event) {
  ...
  try {
    ...
    const payload = {
      app_id: process.env.PLATO_APP_ID, "app-id": process.env.PLATO_APP_ID,
      app_secret: process.env.PLATO_APP_SECRET, "app-secret": process.env.PLATO_APP_SECRET, token
    };
    const r = await fetch("https://clinic.platomedical.com/apps/context", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
    const text = await r.text();
    if (!r.ok) return { statusCode: r.status, headers, body: `Plato context error: ${text}` };
    let ctx = {}; try { ctx = JSON.parse(text); } catch {}
    ...
    if (!patientId || !database) return { statusCode: 200, headers, body: JSON.stringify({ error: "No patient/database in context", ...result }) };
    return { statusCode: 200, headers, body: JSON.stringify(result) };
  } catch (e) { return { statusCode: 500, headers, body: e?.message || "Context failed" }; }
}
